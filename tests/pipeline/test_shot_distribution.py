...
class TestComputeGroupL1Norms:
    def test_single_group_single_term(self):
        assert _compute_group_l1_norms([0.5], [[0]]) == [0.5]

    def test_negative_coefficients_use_absolute_value(self):
        assert _compute_group_l1_norms([-2.0, 3.0], [[0, 1]]) == [5.0]

    def test_multiple_groups(self):
        coeffs = [1.0, -0.5, 2.0, 0.25]
        partition = [[0, 1], [2], [3]]
        assert _compute_group_l1_norms(coeffs, partition) == [1.5, 2.0, 0.25]

    def test_empty_partition(self):
        # No groups -> empty norms list (degenerate but legal).
        assert _compute_group_l1_norms([1.0, 2.0], []) == []


class TestComputeShotDistributionUniform:
    def test_evenly_divisible(self):
        assert _compute_shot_distribution([1.0, 1.0, 1.0], 300, "uniform") == [
            100,
            100,
            100,
        ]
    ...
    def test_remainder_distributed_to_first_groups(self):
        ...
        assert _compute_shot_distribution([1.0, 1.0, 1.0], 10, "uniform") == [4, 3, 3]

    def test_uniform_ignores_norms(self):
        # Even with skewed norms, uniform splits evenly.
        assert _compute_shot_distribution([100.0, 0.01], 10, "uniform") == [5, 5]

    def test_zero_total_shots(self):
        assert _compute_shot_distribution([1.0, 1.0], 0, "uniform") == [0, 0]


class TestComputeShotDistributionWeighted:
    def test_proportional_allocation(self):
        # Weights 3:1, 100 shots -> 75:25
        assert _compute_shot_distribution([3.0, 1.0], 100, "weighted") == [75, 25]
    ...
    def test_largest_remainder_preserves_total(self):
        ...
        result = _compute_shot_distribution([1.0, 1.0, 1.0], 10, "weighted")
    ...
    def test_zero_total_shots(self):
        assert _compute_shot_distribution([2.0, 1.0], 0, "weighted") == [0, 0]

    def test_all_zero_norms_falls_back_to_uniform(self):
        assert _compute_shot_distribution([0.0, 0.0, 0.0], 9, "weighted") == [3, 3, 3]
...
class TestComputeShotDistributionWeightedRandom:
    ...
    def test_zero_total_shots(self):
        assert _compute_shot_distribution(
