...
class TestSpoConversion:
    ...
    def test_to_spo_dict_reads_leftmost_char_as_qubit_zero(self):
        """Dict keys are divi-convention, the reverse of a Qiskit label.

        Probes the convention from two angles: the symplectic representation
        puts X at column 0 (so ``"XI"`` really means ``X(0) I(1)``), and the
        whole operator equals the Qiskit label form with each key reversed.
        """
        ...
        assert (
            spo_dict.simplify()
            == SparsePauliOp.from_list([("IX", 1.0), ("ZI", 0.5)]).simplify()
        )
    ...
    def test_spo_wires_uses_range(self, simple_spo):
        """An SPO carries no wire labels, so its register is ``range``."""
...
def test_to_spo_from_pennylane(qp, simple_spo):
    """A PennyLane Hamiltonian converts to the equivalent native operator."""
    observable = (1.0 * qp.Z(0) + 2.0 * qp.Z(1) + 3.0 * (qp.Z(0) @ qp.Z(1))).simplify()
...
class TestPennyLaneObservableToSparsePauliOp:
    ...
    def test_hamiltonian_sum_of_terms(self, qp):
        wires = qp.wires.Wires([0, 1])
        observable = qp.Hamiltonian([0.5, -0.3], [qp.PauliZ(0), qp.PauliX(1)])
    ...
    def test_sum_of_single_qubit_terms(self, qp):
        wires = qp.wires.Wires([0, 1, 2])
        observable = qp.sum(qp.PauliZ(0), qp.PauliZ(1), qp.PauliZ(2))
        actual = _observable_to_sparse_pauli_op(observable, wires).simplify()
    ...
    def test_to_spo_threads_wires_kwarg(self, qp):
        wires = qp.wires.Wires([0, 1, 2])
        observable = qp.PauliZ(1)
        assert to_spo(observable, wires=wires) == _observable_to_sparse_pauli_op(
            observable, wires
        )
