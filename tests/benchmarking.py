...
class Benchmarking:
    ...
    def hea_ansatz(self, depth=1, entanglement='linear'):
        ...
        self.qc = EfficientSU2(num_qubits=self.num_qubits, entanglement=entanglement, reps=depth)
        self.qc.measure_all()

        # parameter values s.t. -> clifford gates
        angles = [0, -np.pi/2, np.pi/2, -np.pi, np.pi, -(3*np.pi)/2, (3*np.pi)/2, -2*np.pi, 2*np.pi]
